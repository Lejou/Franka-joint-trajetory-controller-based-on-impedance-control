...
#include <franka_effort_controller/ComplianceParamConfig.h>
...
class FrankaForceJointTrajectoryController : public controller_interface::MultiInterfaceController<
...
private:
  ...
  double k_p_{0.5};
  double k_i_{1.2};
  double target_k_p_{0.5};
  double target_k_i_{1.2};
  double filter_gain_{0.001};
  Eigen::Matrix<double, 7, 1> tau_ext_initial_;
  Eigen::Matrix<double, 7, 1> tau_error_;

  ros::Subscriber target_force_sub_;
  ros::Publisher target_force_pub_;
  void desiredForceCB(const std_msgs::Float64& msg);
  ...
  Eigen::Matrix<double, 7, 1> tau_joints_;
  ...
  ros::Publisher test_pub;
	...
	// Dynamic reconfigure
  std::unique_ptr<dynamic_reconfigure::Server<franka_effort_controller::ComplianceParamConfig>>
      dynamic_server_compliance_param_;
  ros::NodeHandle dynamic_reconfigure_compliance_param_node_;
  void complianceParamCallback(franka_effort_controller::ComplianceParamConfig& config,
                               uint32_t level);
  ...
  void getCartStatesRBDyn(Eigen::Vector3d& position, Eigen::Quaterniond& orientation,
    Eigen::Vector3d& position_d, Eigen::Quaterniond& orientation_d); // 还有问题
