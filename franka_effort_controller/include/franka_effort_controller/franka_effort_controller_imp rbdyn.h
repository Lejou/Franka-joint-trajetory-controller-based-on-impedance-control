                                                  ...
                                                  ros::NodeHandle&   controller_nh)
{
  ...
  std::cout <<"nrJoints " << _rbd_indices.size() << std::endl;
...
void FrankaJointTrajectoryController<SegmentImpl>::update(const ros::Time& time, const ros::Duration& period)
{
  ...
  // std::vector<double>& cmd_cal = hw_iface_adapter_.calCommandconst(time_data.uptime, time_data.period,desired_state_, state_error_);
  // cmd_cal.resize(joint_names_.size());
  Eigen::VectorXd tau_d(7);

  for (size_t i = 0; i < 7; ++i) {
    // tau_d(i) = k_gains_[i] * (robot_state.q_d[i] - robot_state.q[i])+ d_gains_[i] * (robot_state.dq_d[i] - dq_filtered_[i]);
...
//   //                 (-cartesian_stiffness_ * error - cartesian_damping_ * (jacobian * (dq - des_dq)));
